...
export function setupQseowDeleteMasterMeasureCommand(qseow) {
    qseow
        ...
        .addOption(
            new Option(
                '--master-item <ids...>',
                'names or IDs of master measures to be deleted. Multiple IDs should be space separated'
            ).env('CTRLQ_MASTER_ITEM')
        )
        .addOption(new Option('--delete-all', 'delete all master measures').env('CTRLQ_DELETE_ALL'))
        .addOption(
