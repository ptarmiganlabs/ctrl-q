...
export function setupQseowGetBookmarkCommand(qseow) {
    qseow
        ...
        .addOption(
            new Option('--bookmark <bookmarks...>', 'bookmark to retrieve. If not specified all bookmarks will be retrieved').env(
                'CTRLQ_BOOKMARK'
            )
        )
        .addOption(
            new Option('--output-format <json|table>', 'output format')
                .choices(['json', 'table'])
                .default('json')
                .env('CTRLQ_OUTPUT_FORMAT')
        );
}
